...
// _AssetService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _AssetService_HTTPInvalidArgument(err error) error {
...
func _AssetService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	...
	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		...
		if err != nil {
			return _AssetService_HTTPInvalidArgument(err)
		}
...
// _AssetService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _AssetService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _AssetService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func RegisterAssetServiceHttpHandlers(router *mux.Router, prefix string, cli AssetServiceClient) {
	...
	router.Handle(prefix+"/assets/bundles", _AssetService_UploadBundle_Rule0(cli)).
		Methods("POST").
		Name("eolymp.asset.AssetService.UploadBundle")
...
func _AssetService_UploadBundle_Rule0(cli AssetServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		in := &UploadBundleInput{}
		...
		out, err := cli.UploadBundle(r.Context(), in, grpc.Header(&header), grpc.Trailer(&trailer))
		if err != nil {
			_AssetService_HTTPWriteErrorResponse(w, err)
			return
		}
