...
var errCodeTemplateServiceRequestTooLarge = errors.New("request too large")

// _CodeTemplateService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _CodeTemplateService_HTTPInvalidArgument(err error) error {
...
// _CodeTemplateService_HTTPReadQueryString parses query string into proto.Message with size limit
func _CodeTemplateService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errCodeTemplateServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _CodeTemplateService_HTTPInvalidArgument(err)
		}
...
// _CodeTemplateService_HTTPReadRequestBody parses body into proto.Message with size limit
func _CodeTemplateService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _CodeTemplateService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _CodeTemplateService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _CodeTemplateService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _CodeTemplateService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errCodeTemplateServiceRequestTooLarge {
...
func _CodeTemplateService_DeleteCodeTemplate_Rule0(cli CodeTemplateServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _CodeTemplateService_HTTPReadRequest(r, in, 1048576, 131072); err != nil {
			_CodeTemplateService_HTTPWriteErrorResponse(w, err)
			return
		}
