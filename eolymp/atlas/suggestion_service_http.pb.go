...
var errSuggestionServiceRequestTooLarge = errors.New("request too large")

// _SuggestionService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _SuggestionService_HTTPInvalidArgument(err error) error {
...
// _SuggestionService_HTTPReadQueryString parses query string into proto.Message with size limit
func _SuggestionService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errSuggestionServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _SuggestionService_HTTPInvalidArgument(err)
		}
...
// _SuggestionService_HTTPReadRequestBody parses body into proto.Message with size limit
func _SuggestionService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _SuggestionService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _SuggestionService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _SuggestionService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _SuggestionService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errSuggestionServiceRequestTooLarge {
...
func _SuggestionService_CreateSuggestion_Rule0(cli SuggestionServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _SuggestionService_HTTPReadRequestBody(r, in, 1048576); err != nil {
			_SuggestionService_HTTPWriteErrorResponse(w, err)
			return
		}
...
func _SuggestionService_DeleteSuggestion_Rule0(cli SuggestionServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _SuggestionService_HTTPReadRequest(r, in, 1048576, 131072); err != nil {
			_SuggestionService_HTTPWriteErrorResponse(w, err)
			return
		}
