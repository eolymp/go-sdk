...
// _ScriptService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _ScriptService_HTTPInvalidArgument(err error) error {
...
func _ScriptService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	...
	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		...
		if err != nil {
			return _ScriptService_HTTPInvalidArgument(err)
		}
...
// _ScriptService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _ScriptService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _ScriptService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _ScriptService_DeleteScript_Rule0(cli ScriptServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _ScriptService_HTTPReadRequest(r, in, 1048576, 131072); err != nil {
			_ScriptService_HTTPWriteErrorResponse(w, err)
			return
		}
