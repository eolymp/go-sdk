...
var errStatementServiceRequestTooLarge = errors.New("request too large")

// _StatementService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _StatementService_HTTPInvalidArgument(err error) error {
...
// _StatementService_HTTPReadQueryString parses query string into proto.Message with size limit
func _StatementService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errStatementServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _StatementService_HTTPInvalidArgument(err)
		}
...
// _StatementService_HTTPReadRequestBody parses body into proto.Message with size limit
func _StatementService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _StatementService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _StatementService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _StatementService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _StatementService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errStatementServiceRequestTooLarge {
...
func _StatementService_CreateStatement_Rule0(cli StatementServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _StatementService_HTTPReadRequestBody(r, in, 1048576); err != nil {
			_StatementService_HTTPWriteErrorResponse(w, err)
			return
		}
...
func _StatementService_DeleteStatement_Rule0(cli StatementServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _StatementService_HTTPReadRequest(r, in, 1048576, 131072); err != nil {
			_StatementService_HTTPWriteErrorResponse(w, err)
			return
		}
...
func _StatementService_ListStatementVersions_Rule0(cli StatementServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		out, err := cli.ListStatementVersions(r.Context(), in, grpc.Header(&header), grpc.Trailer(&trailer))
		if err != nil {
			_StatementService_HTTPWriteErrorResponse(w, err)
			return
		}
