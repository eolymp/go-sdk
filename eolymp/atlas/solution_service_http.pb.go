...
var errSolutionServiceRequestTooLarge = errors.New("request too large")

// _SolutionService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _SolutionService_HTTPInvalidArgument(err error) error {
...
// _SolutionService_HTTPReadQueryString parses query string into proto.Message with size limit
func _SolutionService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errSolutionServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _SolutionService_HTTPInvalidArgument(err)
		}
...
// _SolutionService_HTTPReadRequestBody parses body into proto.Message with size limit
func _SolutionService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _SolutionService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _SolutionService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _SolutionService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _SolutionService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errSolutionServiceRequestTooLarge {
...
func _SolutionService_CreateSolution_Rule0(cli SolutionServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _SolutionService_HTTPReadRequestBody(r, in, 1048576); err != nil {
			_SolutionService_HTTPWriteErrorResponse(w, err)
			return
		}
...
func _SolutionService_DeleteSolution_Rule0(cli SolutionServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _SolutionService_HTTPReadRequest(r, in, 1048576, 131072); err != nil {
			_SolutionService_HTTPWriteErrorResponse(w, err)
			return
		}
