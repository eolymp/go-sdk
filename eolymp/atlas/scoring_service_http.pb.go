...
var errScoringServiceRequestTooLarge = errors.New("request too large")

// _ScoringService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _ScoringService_HTTPInvalidArgument(err error) error {
...
// _ScoringService_HTTPReadQueryString parses query string into proto.Message with size limit
func _ScoringService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errScoringServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _ScoringService_HTTPInvalidArgument(err)
		}
...
// _ScoringService_HTTPReadRequestBody parses body into proto.Message with size limit
func _ScoringService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _ScoringService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _ScoringService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _ScoringService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _ScoringService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errScoringServiceRequestTooLarge {
