...
var errUserServiceRequestTooLarge = errors.New("request too large")

// _UserService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _UserService_HTTPInvalidArgument(err error) error {
...
// _UserService_HTTPReadQueryString parses query string into proto.Message with size limit
func _UserService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errUserServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _UserService_HTTPInvalidArgument(err)
		}
...
// _UserService_HTTPReadRequestBody parses body into proto.Message with size limit
func _UserService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _UserService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _UserService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _UserService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _UserService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errUserServiceRequestTooLarge {
