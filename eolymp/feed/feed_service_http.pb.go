...
var errFeedServiceRequestTooLarge = errors.New("request too large")

// _FeedService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _FeedService_HTTPInvalidArgument(err error) error {
...
// _FeedService_HTTPReadQueryString parses query string into proto.Message with size limit
func _FeedService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errFeedServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _FeedService_HTTPInvalidArgument(err)
		}
...
// _FeedService_HTTPReadRequestBody parses body into proto.Message with size limit
func _FeedService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _FeedService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _FeedService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _FeedService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _FeedService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errFeedServiceRequestTooLarge {
...
func _FeedService_ListEntries_Rule0(cli FeedServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _FeedService_HTTPReadQueryString(r, in, 131072); err != nil {
			_FeedService_HTTPWriteErrorResponse(w, err)
			return
		}
