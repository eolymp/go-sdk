...
var errResolverRequestTooLarge = errors.New("request too large")

// _Resolver_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _Resolver_HTTPInvalidArgument(err error) error {
...
// _Resolver_HTTPReadQueryString parses query string into proto.Message with size limit
func _Resolver_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errResolverRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _Resolver_HTTPInvalidArgument(err)
		}
...
// _Resolver_HTTPReadRequestBody parses body into proto.Message with size limit
func _Resolver_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _Resolver_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _Resolver_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _Resolver_HTTPReadQueryString(r, v, queryMaxSize)
}

// _Resolver_HTTPWriteResponse writes proto.Message to HTTP response
func _Resolver_HTTPWriteResponse(w http.ResponseWriter, v proto.Message, h, t metadata.MD) {
...
func _Resolver_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errResolverRequestTooLarge {
...
// RegisterResolverHttpProxy adds proxy handlers for for ResolverClient
func RegisterResolverHttpProxy(router *mux.Router, prefix string, conn grpc.ClientConnInterface) {
	RegisterResolverHttpHandlers(router, prefix, NewResolverClient(conn))
}
...
func _Resolver_ResolveName_Rule0(cli ResolverClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _Resolver_HTTPReadQueryString(r, in, 131072); err != nil {
			_Resolver_HTTPWriteErrorResponse(w, err)
			return
		}
		...
		out, err := cli.ResolveName(r.Context(), in, grpc.Header(&header), grpc.Trailer(&trailer))
		if err != nil {
			_Resolver_HTTPWriteErrorResponse(w, err)
			return
		}
