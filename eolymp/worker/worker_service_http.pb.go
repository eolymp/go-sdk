...
var errWorkerServiceRequestTooLarge = errors.New("request too large")

// _WorkerService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _WorkerService_HTTPInvalidArgument(err error) error {
...
// _WorkerService_HTTPReadQueryString parses query string into proto.Message with size limit
func _WorkerService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errWorkerServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _WorkerService_HTTPInvalidArgument(err)
		}
...
// _WorkerService_HTTPReadRequestBody parses body into proto.Message with size limit
func _WorkerService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _WorkerService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _WorkerService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _WorkerService_HTTPReadQueryString(r, v, queryMaxSize)
}

// _WorkerService_HTTPWriteResponse writes proto.Message to HTTP response
func _WorkerService_HTTPWriteResponse(w http.ResponseWriter, v proto.Message, h, t metadata.MD) {
...
func _WorkerService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errWorkerServiceRequestTooLarge {
...
func _WorkerService_DescribeJob_Rule0(cli WorkerServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		out, err := cli.DescribeJob(r.Context(), in, grpc.Header(&header), grpc.Trailer(&trailer))
		if err != nil {
			_WorkerService_HTTPWriteErrorResponse(w, err)
			return
		}

		_WorkerService_HTTPWriteResponse(w, out, header, trailer)
	})
}
...
func _WorkerService_ListJobs_Rule0(cli WorkerServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		out, err := cli.ListJobs(r.Context(), in, grpc.Header(&header), grpc.Trailer(&trailer))
		if err != nil {
			_WorkerService_HTTPWriteErrorResponse(w, err)
			return
		}
