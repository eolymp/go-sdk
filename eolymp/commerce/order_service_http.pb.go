...
var errOrderServiceRequestTooLarge = errors.New("request too large")

// _OrderService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _OrderService_HTTPInvalidArgument(err error) error {
...
// _OrderService_HTTPReadQueryString parses query string into proto.Message with size limit
func _OrderService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errOrderServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _OrderService_HTTPInvalidArgument(err)
		}
...
// _OrderService_HTTPReadRequestBody parses body into proto.Message with size limit
func _OrderService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _OrderService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _OrderService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _OrderService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _OrderService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errOrderServiceRequestTooLarge {
