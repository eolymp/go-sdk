...
var errInstitutionServiceRequestTooLarge = errors.New("request too large")

// _InstitutionService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _InstitutionService_HTTPInvalidArgument(err error) error {
...
// _InstitutionService_HTTPReadQueryString parses query string into proto.Message with size limit
func _InstitutionService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errInstitutionServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _InstitutionService_HTTPInvalidArgument(err)
		}
...
// _InstitutionService_HTTPReadRequestBody parses body into proto.Message with size limit
func _InstitutionService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _InstitutionService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _InstitutionService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _InstitutionService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _InstitutionService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errInstitutionServiceRequestTooLarge {
