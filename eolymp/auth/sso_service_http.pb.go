...
// _SSOService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _SSOService_HTTPInvalidArgument(err error) error {
...
func _SSOService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	...
	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		...
		if err != nil {
			return _SSOService_HTTPInvalidArgument(err)
		}
...
// _SSOService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _SSOService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _SSOService_HTTPReadQueryString(r, v, queryMaxSize)
}

// _SSOService_HTTPWriteResponse writes proto.Message to HTTP response
func _SSOService_HTTPWriteResponse(w http.ResponseWriter, v proto.Message, h, t metadata.MD) {
...
func RegisterSSOServiceHttpProxy(router *mux.Router, prefix string, conn grpc.ClientConnInterface) {
...
func _SSOService_SignonRequest_Rule0(cli SSOServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_SSOService_HTTPWriteResponse(w, out, header, trailer)
	})
}
