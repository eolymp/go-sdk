...
// _ContentService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _ContentService_HTTPInvalidArgument(err error) error {
...
func _ContentService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	...
	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		...
		if err != nil {
			return _ContentService_HTTPInvalidArgument(err)
		}
...
// _ContentService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _ContentService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _ContentService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func RegisterContentServiceHttpHandlers(router *mux.Router, prefix string, cli ContentServiceClient) {
	router.Handle(prefix+"/contests/{contest_id}/content/fragments/{fragment_id}", _ContentService_DescribeFragment_Rule0(cli)).
		Methods("GET").
		Name("eolymp.judge.ContentService.DescribeFragment")
	router.Handle(prefix+"/contests/{contest_id}/content/fragments", _ContentService_ListFragments_Rule0(cli)).
		Methods("GET").
		Name("eolymp.judge.ContentService.ListFragments")
	router.Handle(prefix+"/contests/{contest_id}/content/fragments", _ContentService_CreateFragment_Rule0(cli)).
		Methods("POST").
		Name("eolymp.judge.ContentService.CreateFragment")
	router.Handle(prefix+"/contests/{contest_id}/content/fragments/{fragment_id}", _ContentService_UpdateFragment_Rule0(cli)).
		Methods("PUT").
		Name("eolymp.judge.ContentService.UpdateFragment")
	router.Handle(prefix+"/contests/{contest_id}/content/fragments/{fragment_id}", _ContentService_DeleteFragment_Rule0(cli)).
		Methods("DELETE").
		Name("eolymp.judge.ContentService.DeleteFragment")
	router.Handle(prefix+"/contests/{contest_id}/content/fragments/{fragment_id}/translate", _ContentService_TranslateFragment_Rule0(cli)).
		Methods("POST").
		Name("eolymp.judge.ContentService.TranslateFragment")
	router.Handle(prefix+"/contests/{contest_id}/content/path", _ContentService_DescribePath_Rule0(cli)).
		Methods("GET").
		Name("eolymp.judge.ContentService.DescribePath")
	router.Handle(prefix+"/contests/{contest_id}/content/parents", _ContentService_ListParents_Rule0(cli)).
		Methods("GET").
		Name("eolymp.judge.ContentService.ListParents")
}
...
func _ContentService_DeleteFragment_Rule0(cli ContentServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _ContentService_HTTPReadRequest(r, in, 1048576, 131072); err != nil {
			_ContentService_HTTPWriteErrorResponse(w, err)
			return
		}
