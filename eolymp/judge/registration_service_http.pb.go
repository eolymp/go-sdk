...
var errRegistrationServiceRequestTooLarge = errors.New("request too large")

// _RegistrationService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _RegistrationService_HTTPInvalidArgument(err error) error {
...
// _RegistrationService_HTTPReadQueryString parses query string into proto.Message with size limit
func _RegistrationService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errRegistrationServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _RegistrationService_HTTPInvalidArgument(err)
		}
...
// _RegistrationService_HTTPReadRequestBody parses body into proto.Message with size limit
func _RegistrationService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _RegistrationService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _RegistrationService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _RegistrationService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _RegistrationService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errRegistrationServiceRequestTooLarge {
...
func _RegistrationService_DescribeRegistration_Rule0(cli RegistrationServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _RegistrationService_HTTPReadQueryString(r, in, 131072); err != nil {
			_RegistrationService_HTTPWriteErrorResponse(w, err)
			return
		}
...
func _RegistrationService_SubmitRegistration_Rule0(cli RegistrationServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _RegistrationService_HTTPReadRequestBody(r, in, 1048576); err != nil {
			_RegistrationService_HTTPWriteErrorResponse(w, err)
			return
		}
