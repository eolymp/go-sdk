...
var errMessageServiceRequestTooLarge = errors.New("request too large")

// _MessageService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _MessageService_HTTPInvalidArgument(err error) error {
...
// _MessageService_HTTPReadQueryString parses query string into proto.Message with size limit
func _MessageService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errMessageServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _MessageService_HTTPInvalidArgument(err)
		}
...
// _MessageService_HTTPReadRequestBody parses body into proto.Message with size limit
func _MessageService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _MessageService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _MessageService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _MessageService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _MessageService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errMessageServiceRequestTooLarge {
...
func RegisterMessageServiceHttpHandlers(router *mux.Router, prefix string, cli MessageServiceClient) {
	router.Handle(prefix+"/messages", _MessageService_CreateMessage_Rule0(cli)).
		Methods("POST").
		Name("eolymp.notify.MessageService.CreateMessage")
	router.Handle(prefix+"/messages/{message_id}", _MessageService_UpdateMessage_Rule0(cli)).
		Methods("PUT").
		Name("eolymp.notify.MessageService.UpdateMessage")
	router.Handle(prefix+"/messages/{message_id}", _MessageService_DeleteMessage_Rule0(cli)).
		Methods("DELETE").
		Name("eolymp.notify.MessageService.DeleteMessage")
	router.Handle(prefix+"/messages/{message_id}", _MessageService_DescribeMessage_Rule0(cli)).
		Methods("GET").
		Name("eolymp.notify.MessageService.DescribeMessage")
	router.Handle(prefix+"/messages", _MessageService_ListMessages_Rule0(cli)).
		Methods("GET").
		Name("eolymp.notify.MessageService.ListMessages")
	router.Handle(prefix+"/messages/{message_id}/publish", _MessageService_PublishMessage_Rule0(cli)).
		Methods("POST").
		Name("eolymp.notify.MessageService.PublishMessage")
}
...
func _MessageService_CreateMessage_Rule0(cli MessageServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		in := &CreateMessageInput{}

		if err := _MessageService_HTTPReadRequestBody(r, in, 1048576); err != nil {
			_MessageService_HTTPWriteErrorResponse(w, err)
			return
		}
		...
		out, err := cli.CreateMessage(r.Context(), in, grpc.Header(&header), grpc.Trailer(&trailer))
		if err != nil {
			_MessageService_HTTPWriteErrorResponse(w, err)
			return
		}
...
func _MessageService_DeleteMessage_Rule0(cli MessageServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _MessageService_HTTPReadRequest(r, in, 1048576, 131072); err != nil {
			_MessageService_HTTPWriteErrorResponse(w, err)
			return
		}
...
func _MessageService_PublishMessage_Rule0(cli MessageServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		in := &PublishMessageInput{}
		...
		out, err := cli.PublishMessage(r.Context(), in, grpc.Header(&header), grpc.Trailer(&trailer))
		if err != nil {
			_MessageService_HTTPWriteErrorResponse(w, err)
			return
		}
