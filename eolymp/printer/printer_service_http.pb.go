...
var errPrinterServiceRequestTooLarge = errors.New("request too large")

// _PrinterService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _PrinterService_HTTPInvalidArgument(err error) error {
...
// _PrinterService_HTTPReadQueryString parses query string into proto.Message with size limit
func _PrinterService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errPrinterServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _PrinterService_HTTPInvalidArgument(err)
		}
...
// _PrinterService_HTTPReadRequestBody parses body into proto.Message with size limit
func _PrinterService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _PrinterService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _PrinterService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _PrinterService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _PrinterService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errPrinterServiceRequestTooLarge {
...
func _PrinterService_CreatePrinter_Rule0(cli PrinterServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _PrinterService_HTTPReadRequestBody(r, in, 1048576); err != nil {
			_PrinterService_HTTPWriteErrorResponse(w, err)
			return
		}
...
func _PrinterService_DeletePrinter_Rule0(cli PrinterServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _PrinterService_HTTPReadRequest(r, in, 1048576, 131072); err != nil {
			_PrinterService_HTTPWriteErrorResponse(w, err)
			return
		}
