...
// _ProfileService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _ProfileService_HTTPInvalidArgument(err error) error {
...
func _ProfileService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	...
	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		...
		if err != nil {
			return _ProfileService_HTTPInvalidArgument(err)
		}
...
// _ProfileService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _ProfileService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _ProfileService_HTTPReadQueryString(r, v, queryMaxSize)
}
