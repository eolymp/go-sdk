...
var errAccessKeyServiceRequestTooLarge = errors.New("request too large")

// _AccessKeyService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _AccessKeyService_HTTPInvalidArgument(err error) error {
...
// _AccessKeyService_HTTPReadQueryString parses query string into proto.Message with size limit
func _AccessKeyService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errAccessKeyServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _AccessKeyService_HTTPInvalidArgument(err)
		}
...
// _AccessKeyService_HTTPReadRequestBody parses body into proto.Message with size limit
func _AccessKeyService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _AccessKeyService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _AccessKeyService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _AccessKeyService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _AccessKeyService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errAccessKeyServiceRequestTooLarge {
...
func _AccessKeyService_DeleteAccessKey_Rule0(cli AccessKeyServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _AccessKeyService_HTTPReadRequest(r, in, 1048576, 131072); err != nil {
			_AccessKeyService_HTTPWriteErrorResponse(w, err)
			return
		}
