...
var errCreditServiceRequestTooLarge = errors.New("request too large")

// _CreditService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _CreditService_HTTPInvalidArgument(err error) error {
...
// _CreditService_HTTPReadQueryString parses query string into proto.Message with size limit
func _CreditService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errCreditServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _CreditService_HTTPInvalidArgument(err)
		}
...
// _CreditService_HTTPReadRequestBody parses body into proto.Message with size limit
func _CreditService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _CreditService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _CreditService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _CreditService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _CreditService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errCreditServiceRequestTooLarge {
...
func _CreditService_GrantCredit_Rule0(cli CreditServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _CreditService_HTTPReadRequestBody(r, in, 1048576); err != nil {
			_CreditService_HTTPWriteErrorResponse(w, err)
			return
		}
...
func _CreditService_CancelCredit_Rule0(cli CreditServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _CreditService_HTTPReadRequest(r, in, 1048576, 131072); err != nil {
			_CreditService_HTTPWriteErrorResponse(w, err)
			return
		}
