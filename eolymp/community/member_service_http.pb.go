...
// _MemberService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _MemberService_HTTPInvalidArgument(err error) error {
...
func _MemberService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	...
	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		...
		if err != nil {
			return _MemberService_HTTPInvalidArgument(err)
		}
...
// _MemberService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _MemberService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _MemberService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func RegisterMemberServiceHttpHandlers(router *mux.Router, prefix string, cli MemberServiceClient) {
	...
	router.Handle(prefix+"/members/{member_id}/login-link", _MemberService_CreateMemberLoginLink_Rule0(cli)).
		Methods("POST").
		Name("eolymp.community.MemberService.CreateMemberLoginLink")
...
func _MemberService_DeleteMember_Rule0(cli MemberServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _MemberService_HTTPReadRequest(r, in, 1048576, 131072); err != nil {
			_MemberService_HTTPWriteErrorResponse(w, err)
			return
		}
...
func _MemberService_CreateMemberLoginLink_Rule0(cli MemberServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		in := &CreateMemberLoginLinkInput{}
		...
		out, err := cli.CreateMemberLoginLink(r.Context(), in, grpc.Header(&header), grpc.Trailer(&trailer))
		if err != nil {
			_MemberService_HTTPWriteErrorResponse(w, err)
			return
		}
