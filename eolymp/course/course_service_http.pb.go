...
var errCourseServiceRequestTooLarge = errors.New("request too large")

// _CourseService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _CourseService_HTTPInvalidArgument(err error) error {
...
// _CourseService_HTTPReadQueryString parses query string into proto.Message with size limit
func _CourseService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errCourseServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _CourseService_HTTPInvalidArgument(err)
		}
...
// _CourseService_HTTPReadRequestBody parses body into proto.Message with size limit
func _CourseService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _CourseService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _CourseService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _CourseService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _CourseService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errCourseServiceRequestTooLarge {
...
func _CourseService_CreateCourse_Rule0(cli CourseServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _CourseService_HTTPReadRequestBody(r, in, 1048576); err != nil {
			_CourseService_HTTPWriteErrorResponse(w, err)
			return
		}
...
func _CourseService_DeleteCourse_Rule0(cli CourseServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _CourseService_HTTPReadRequest(r, in, 1048576, 131072); err != nil {
			_CourseService_HTTPWriteErrorResponse(w, err)
			return
		}
