...
var errModuleServiceRequestTooLarge = errors.New("request too large")

// _ModuleService_HTTPInvalidArgument wraps a request parsing error into an InvalidArgument status
// with structured details so it is reported as HTTP 400 instead of 500.
func _ModuleService_HTTPInvalidArgument(err error) error {
...
// _ModuleService_HTTPReadQueryString parses query string into proto.Message with size limit
func _ModuleService_HTTPReadQueryString(r *http.Request, v proto.Message, maxSize int64) error {
	if int64(len(r.URL.RawQuery)) > maxSize {
		return errModuleServiceRequestTooLarge
	}

	if h := r.Header.Values("Strict-Parsing"); len(h) > 0 {
		strict, err := strconv.ParseBool(h[len(h)-1])
		if err != nil {
			return _ModuleService_HTTPInvalidArgument(err)
		}
...
// _ModuleService_HTTPReadRequestBody parses body into proto.Message with size limit
func _ModuleService_HTTPReadRequestBody(r *http.Request, v proto.Message, maxSize int64) error {
...
// _ModuleService_HTTPReadRequest reads request parameters from the body when it is present,
// otherwise it falls back to the query string. Used for verbs (DELETE) that may carry parameters either way.
func _ModuleService_HTTPReadRequest(r *http.Request, v proto.Message, bodyMaxSize, queryMaxSize int64) error {
	...
	return _ModuleService_HTTPReadQueryString(r, v, queryMaxSize)
}
...
func _ModuleService_HTTPWriteErrorResponse(w http.ResponseWriter, e error) {
	...
	if e == errModuleServiceRequestTooLarge {
...
func _ModuleService_CreateModule_Rule0(cli ModuleServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _ModuleService_HTTPReadRequestBody(r, in, 1048576); err != nil {
			_ModuleService_HTTPWriteErrorResponse(w, err)
			return
		}
...
func _ModuleService_DeleteModule_Rule0(cli ModuleServiceClient) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := _ModuleService_HTTPReadRequest(r, in, 1048576, 131072); err != nil {
			_ModuleService_HTTPWriteErrorResponse(w, err)
			return
		}
