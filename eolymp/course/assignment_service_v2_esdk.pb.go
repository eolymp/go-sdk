// Code generated by protoc-gen-go-esdk. DO NOT EDIT.
// See https://github.com/eolymp/contracts/tree/main/cmd/protoc-gen-go-esdk for more details.

package course

import (
	bytes "bytes"
	context "context"
	fmt "fmt"
	protojson "google.golang.org/protobuf/encoding/protojson"
	proto "google.golang.org/protobuf/proto"
	io "io"
	ioutil "io/ioutil"
	http "net/http"
	url "net/url"
	os "os"
)

type _AssignmentServiceV2HttpClient interface {
	Do(*http.Request) (*http.Response, error)
}

type AssignmentServiceV2Service struct {
	base string
	cli  _AssignmentServiceV2HttpClient
}

// NewAssignmentServiceV2HttpClient constructs client for AssignmentServiceV2
func NewAssignmentServiceV2HttpClient(url string, cli _AssignmentServiceV2HttpClient) *AssignmentServiceV2Service {
	if url == "" {
		url = os.Getenv("EOLYMP_API_URL")
		if url == "" {
			url = "https://api.eolymp.com"
		}
	}

	return &AssignmentServiceV2Service{base: url, cli: cli}
}

func (s *AssignmentServiceV2Service) do(ctx context.Context, verb, path string, in, out proto.Message) (err error) {
	var body io.Reader

	if in != nil {
		data, err := protojson.Marshal(in)
		if err != nil {
			return err
		}

		if verb != "GET" {
			body = bytes.NewReader(data)
		} else {
			query := url.Values{"q": []string{string(data)}}
			path = path + "?" + query.Encode()
		}
	}

	if in == nil && verb != "GET" {
		body = bytes.NewReader([]byte("{}"))
	}

	req, err := http.NewRequest(verb, s.base+path, body)
	if err != nil {
		return err
	}

	req.Header.Set("Accept", "application/json")
	req.Header.Set("Content-Type", "application/json")

	req = req.WithContext(ctx)

	resp, err := s.cli.Do(req)
	if resp != nil && resp.Body != nil {
		defer resp.Body.Close()
	}

	if err != nil {
		return err
	}

	if resp.StatusCode != 200 {
		data, err := ioutil.ReadAll(resp.Body)
		if err != nil {
			return fmt.Errorf("non-200 response code (%v)", resp.StatusCode)
		}

		return fmt.Errorf("non-200 response code (%v): %s", resp.StatusCode, data)
	}

	if out != nil {
		data, err := ioutil.ReadAll(resp.Body)
		if err != nil {
			return err
		}

		if err := protojson.Unmarshal(data, out); err != nil {
			return err
		}
	}

	return nil
}

func (s *AssignmentServiceV2Service) CreateAssignment(ctx context.Context, in *CreateAssignmentV2Input) (*CreateAssignmentV2Output, error) {
	out := &CreateAssignmentV2Output{}
	path := "/v2/assignments"

	if err := s.do(ctx, "POST", path, in, out); err != nil {
		return nil, err
	}

	return out, nil
}

func (s *AssignmentServiceV2Service) UpdateAssignment(ctx context.Context, in *UpdateAssignmentV2Input) (*UpdateAssignmentV2Output, error) {
	out := &UpdateAssignmentV2Output{}
	path := "/v2/assignments/" + url.PathEscape(in.GetAssignmentId())

	// Cleanup URL parameters to avoid any ambiguity
	if in != nil {
		in.AssignmentId = ""
	}

	if err := s.do(ctx, "PUT", path, in, out); err != nil {
		return nil, err
	}

	return out, nil
}

func (s *AssignmentServiceV2Service) DeleteAssignment(ctx context.Context, in *DeleteAssignmentV2Input) (*DeleteAssignmentV2Output, error) {
	out := &DeleteAssignmentV2Output{}
	path := "/v2/assignments/" + url.PathEscape(in.GetAssignmentId())

	// Cleanup URL parameters to avoid any ambiguity
	if in != nil {
		in.AssignmentId = ""
	}

	if err := s.do(ctx, "DELETE", path, in, out); err != nil {
		return nil, err
	}

	return out, nil
}

func (s *AssignmentServiceV2Service) DescribeAssignment(ctx context.Context, in *DescribeAssignmentV2Input) (*DescribeAssignmentV2Output, error) {
	out := &DescribeAssignmentV2Output{}
	path := "/v2/assignments/" + url.PathEscape(in.GetAssignmentId())

	// Cleanup URL parameters to avoid any ambiguity
	if in != nil {
		in.AssignmentId = ""
	}

	if err := s.do(ctx, "GET", path, in, out); err != nil {
		return nil, err
	}

	return out, nil
}

func (s *AssignmentServiceV2Service) ListAssignments(ctx context.Context, in *ListAssignmentsV2Input) (*ListAssignmentsV2Output, error) {
	out := &ListAssignmentsV2Output{}
	path := "/v2/assignments"

	if err := s.do(ctx, "GET", path, in, out); err != nil {
		return nil, err
	}

	return out, nil
}
